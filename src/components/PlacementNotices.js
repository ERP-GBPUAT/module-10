...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case "FETCH_PDF_SUCCESS":
      return { ...state, pdfData: action.payload, loading: false };
    case "FETCH_FAIL":
...
const PlacementNotices = () => {
  ...
  const [{ loading, error, notices,pdfData }, dispatch] = useReducer(reducer, {
    loading: true,
    notices: [],
    error: "",
    pdfData:null,
      ...
      try {
        const res = await fetch(
          `http://localhost:8080/notices/getNoticeByType`,
          {
            ...
            body: JSON.stringify({ type: "Placement notice" }),
          }
        );
